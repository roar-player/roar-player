...
<template>
	<div>
		...
		<div class="bb-pattern-player-container" ref="containerRef">
			<table class="bb-pattern-player" :class="[`time-${pattern.time}`, { 'fixed-column-widths': config.areColumnWidthsFixedInPlayer }, { 'readonly': readonly }]">
				<thead>
					<tr>
	...
	.bb-pattern-player-container {
		...
		.bb-pattern-player {
			...
			&.time-12 {
				...
				/* For Listen mode, use smaller column widths to make bars shorter.
				In theory stroke names can now overlap.
				In practice we assume that you can't play faster than 8 strokes per beat in binary time, and 6 in ternary time. */
				&.readonly .stroke-inner {
					min-width: 0.5ex;
				}
				&.readonly.fixed-column-widths .stroke-inner {
					width: 0.5ex;
					min-width: unset;
				}
